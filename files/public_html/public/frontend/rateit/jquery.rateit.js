    ...
    $.fn.rateit = function (p1, p2) {
        ...
        return this.each(function () {
            ...
            var itemdata = function (key, value) {
                ...
                if (value != null) {
                    //update Area values
                    var Areakey = 'Area-value' + ((key == 'value') ? 'now' : key);
                    var range = item.find('.rateit-range');
                    if (range.attr(Areakey) != undefined) {
                        range.attr(Areakey, value);
                    }
            ...
            if (!itemdata('init')) {
                ...
                //Create the necessary tags. For Area purposes we need to give the items an ID. So we use an internal index to create unique ids
                var element = item[0].nodeName == 'DIV' ? 'div' : 'span';
                index++;

                // tabindex="0" gets only added in readonly mode. When keyboard tabbing, no focus is needed in readonly mode.
                var html = '<button id="rateit-reset-{{index}}" type="button" data-role="none" class="rateit-reset" Area-label="' + $.rateit.Area.resetLabel + '" Area-controls="rateit-range-{{index}}"><span></span></button><{{element}} id="rateit-range-{{index}}" class="rateit-range"' + (itemdata('readonly') == true ? '' : ' tabindex="0"') + ' role="slider" Area-label="' + $.rateit.Area.ratingLabel + '" Area-owns="rateit-reset-{{index}}" Area-valuemin="' + itemdata('min') + '" Area-valuemax="' + itemdata('max') + '" Area-valuenow="' + itemdata('value') + '"><{{element}} class="rateit-empty"></{{element}}><{{element}} class="rateit-selected"></{{element}}><{{element}} class="rateit-hover"></{{element}}></{{element}}>';
            ...
            range.attr('Area-readonly', itemdata('readonly'));
        });
    };
