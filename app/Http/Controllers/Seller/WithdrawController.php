...
class WithdrawController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (Hash::check($request->password, auth('seller')->user()->password)) {
            if (auth('seller')->user()->wallet >= $request->amount) {
                ...
                auth('seller')->user()->update([
                    'wallet' => auth('seller')->user()->wallet - $request->amount
                ]);

                // Notify to admin
                $this->AdminNotification($withdraw->id, route('backend.withdraws.index', ['withdraw' => $withdraw->id]), __('New withdraw request.'));

                return response()->json([
                    'redirect' => route('seller.withdraws.index'),
                    'message' => "Withdraw request created successfully. Please wait for approval."
                ]);
            } else {
                return response()->json('Insufficient balance. Your balance is ' . (auth('seller')->user()->wallet ?? 0), 404);
            }
        } else {
